...
public class Bank {
	 ...
	 * <p>
	 * Creates a new bank object that is to be used to host client
	...
	private void loginMenu(int id) {
		System.out.println(
				"Account options for account: " + this.clients[id].getName() + " - $" + this.clients[id].getBalance());
	...
	private void create() {
		if (this.numBankAccounts != 5) {
			...
			if (nameExists(name, this.clients))
				return;
