...
public class PictureExplorer implements MouseMotionListener, ActionListener, MouseListener {
	...
	public static void convertToBlackAndWhite() {
		...
		for (Pixel[] rowArray : pix) {
			for (Pixel pix1 : rowArray) {
				double avg = pix1.getAverage();
				pix1.setBlue((int) avg);
				pix1.setGreen((int) avg);
				pix1.setRed((int) avg);
			}
		}
		pic.explore();
	...
	public static void adjustBrightness(double x) {
		...
		for (Pixel[] rowArray : pix) {
			for (Pixel pixObj : rowArray) {
				int r = pixObj.getRed(), g = pixObj.getGreen(), b = pixObj.getBlue();
				largest = Math.max(r, Math.max(g,b));
				double c = x;
				if (largest * c > 255) {
					c = 255.0/largest;
				}
				
				pixObj.setRed((int) (r * c));
				pixObj.setGreen((int) (g * c));
				pixObj.setBlue((int) (b * c));
			}
		}
		pic.explore();
	}
	...
	public static void count() {
		int count = 0;
		int[][] arr = new int[36][22];
		for (int row = 34; row < arr.length; row++) {
			for (int col = 10; col < arr[0].length; col++) {
				count++;
			}
		}
